...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  List searchedMessages=[];

  searchOrders(String item){
    print('chla');
    setState(() {
      List temp=list;
      ...
      temp.forEach((element) {
        print(element.title);
        if(element.title!.toString().toLowerCase().contains(item.toLowerCase()))
          list.add(element);
      });
    });
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(18.0),
                        child: list[index].imageUrl=="f"? Container( height: h * 0.101,width: 70,):Image.file(
...
*/
  ...
  void loadData() {
    ...
    if (listString != null) {
      ...
      //   double diff = checkexp(dateTimeNow, list[i].diff!,
      //       list[i].expdate!);
      //
      //   if(diff<=0)
      //     exp.add(list[i]);
      ...
      //
      // list=[];
      // exp.forEach((element) {list.add(element);});
      // nonExp.forEach((element) {list.add(element);});


      list.sort((a,b) => a.expdate!.compareTo(b.expdate!));
      setState(() {});
    }
  }
  checkexp(DateTime from, double d, String too) {
    //  from = DateTime(from.year, from.month, from.day);
    //  to = DateTime(to.year, to.month, to.day);
